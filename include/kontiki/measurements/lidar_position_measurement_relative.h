...
#pragma once
#ifndef KONTIKIV2_LIDAR_POSITION_MEASUREMENT_RELATIVE_H
#define KONTIKIV2_LIDAR_POSITION_MEASUREMENT_RELATIVE_H
...
class LiDARPositionMeasurementRelative {
  using Vector3 = Eigen::Matrix<double, 3, 1>;
 public:
  LiDARPositionMeasurementRelative(std::shared_ptr<LiDARModel> lidar, double t, const Vector3 &p, double t0, double weight=1.0, double coster_th=7.8)
    : lidar_(lidar), t_(t), t0_(t0), p_(p), weight_(weight), huber_coster_th_(coster_th), huber_coster_(coster_th>=0.0?coster_th:1e38) {}
  ...
  Eigen::Matrix<T, 3, 1> Measure(const type::Trajectory<TrajectoryModel, T> &trajectory,
    ...
    const Eigen::Matrix<T, 3, 1> p_L_I = q_I_L.conjugate() * (-p_I_L);

    // const Eigen::Matrix<T, 3, 1> p_L = p_.cast<T>();
    const Eigen::Matrix<T, 3, 1> p_L = Eigen::Matrix<T, 3, 1>::Zero();
    const Eigen::Matrix<T, 3, 1> p_I = q_I_L * p_L + p_I_L;
    const Eigen::Matrix<T, 3, 1> p_M = T_M_I->orientation * p_I + T_M_I->position;
    const Eigen::Matrix<T, 3, 1> p_I0 = T_M_I0->orientation.conjugate() * (p_M - T_M_I0->position);
    const Eigen::Matrix<T, 3, 1> p_L0 = q_L_I * p_I0 + p_L_I;
  ...
  Eigen::Matrix<T, 3, 1> ErrorRaw(const type::Trajectory<TrajectoryModel, T> &trajectory,
                               const type::LiDAR<LiDARModel, T> &lidar) const {
    return p_ - Measure<TrajectoryModel, T>(trajectory, lidar);
  }
 ...
 protected:
  ...
  template<typename TrajectoryModel>
  struct Residual {
    Residual(const LiDARPositionMeasurementRelative<LiDARModel> &m) : measurement(m) {}
    ...
    const LiDARPositionMeasurementRelative& measurement;
